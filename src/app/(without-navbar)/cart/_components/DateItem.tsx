...
export default function DateItem({ day, isSelected, isToday=false,onSelect }: DateItemProps) {
  return (
    <div
      className={cn(
        " border border-border size-24 px-8 py-2 rounded-3xl flex flex-col items-center justify-evenly text-accent-foreground cursor-pointer",
        "transition-all duration-400 ease-in-out",
        isSelected && "bg-gradient text-accent",
      )}
      onClick={() => onSelect(day.date.day)}
    >
      <span className={cn("text-muted-foreground text-sm font-light", isSelected && "text-accent")}>{day.date.monthName}</span>
