...
export default function GiftPersonality() {
  const giftPersonalityData = useGiftAssistantStore(
    (store) => store.giftPersonality,
  );
  const setAge = useGiftAssistantStore((store) => store.setAge);
  const setEventId = useGiftAssistantStore((store) => store.setEventId);
  return (
    <>
      <GiftPersonAvatar
      ...
      <div className="flex-between mb-5">
        ...
        <div>
          <span className="font-semibold text-xl ml-4">
            {toPersianDigits(giftPersonalityData.age)}
          </span>
      ...
      <div>
        <ImageSelect
          ...
          selectedId={giftPersonalityData.eventId}
          onSelect={(id) => {
            setEventId(id);
          }}
        />
